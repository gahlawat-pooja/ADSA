...
class BST:
    ...
    def _delete(self, node, key):
        ...
        if key < node.key:
            ...
            node.right = self._delete(node.left, key)
        else:
            if not node.right:
                return node.left
            elif not node.left:
                return node.right
            temp = self._minValueNode(node.left)
            node.key = temp.key
    ...
    def _rightRotate(self, node, key):
        if not node:
            return None
        if key < node.key:
            node.right = self._rightRotate(node.left, key)
        elif key > node.key:
            node.left = self._rightRotate(node.right, key)
        else:
            if not node.right:
                return node
            new_root = node.right
